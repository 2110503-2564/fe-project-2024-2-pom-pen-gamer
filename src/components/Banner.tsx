...
autoPlay loop muted/>
<div className='relative px-5 py-4 mt-24 z-20 text-center text-pink-100 shadow-lg '> 
    <h1 className={styles.text_shadow} >Pom Pen Gamer</h1>
<div>
    <h3 className={styles.text_smaller}> A Restaurant Reservation for Gamer & Cat Lover</h3> </div>
</div>

{/* {
    session ? <div className='z-30 absolute top-5 right-10 font-semibold text-white text-xl'>  Welcome {session.user.name} </div> : null
} */}
   <div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30  mt-10">
...
onClick={(e)=>{e.stopPropagation(); router.push('/restaurant')}}>Select Restaurant</button>
   </div>
</div>
